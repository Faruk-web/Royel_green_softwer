...
class PurchaseInvoiceController extends Controller
{
    ...
    public function purchase_invoice($invoice_id)
    {
        ...
        if($btb_invoice_info) {
            return view('purchase.view_invoice', compact('business_info', 'btb_invoice_info'));
        }
        else {
    ...
    //purchase chalan view
    public function purchase_chalan_view($invoice_id)
    {
        $business_info = DB::table('business_infos')->first();
        $btb_invoice_info = PurchaseInvoice::where('id', $invoice_id)->first();
        if($btb_invoice_info) {
            return view('purchase.view_chalan', compact('business_info', 'btb_invoice_info'));
        }
        else {
            return Redirect()->back()->with('error', 'Sorry you can not access this page');
        }
    ...
    public function Invoice_list_data(Request $request){
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ->addIndexColumn()
                ->addColumn('action', function($row){
                    return '<a href="'.route('purchase.stock.in.view.invoice', ['invoice_id'=>$row->id]).'" class="btn btn-primary btn-sm btn-rounded">View Invoice</a> <a href="'.route('purchase.stock.in.view.chalan', ['invoice_id'=>$row->id]).'" class="btn btn-success btn-sm btn-rounded">Chalan</a>';
                })
